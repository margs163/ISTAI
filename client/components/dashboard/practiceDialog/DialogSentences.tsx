import { SentenceImprovementsType } from "@/lib/types";
import {
  CircleCheck,
  CircleX,
  GanttChartSquare,
...
export function DialogImprovement({
  improvement,
}: {
  improvement: {
  ...
  return (
    <div className="border border-gray-200 bg-white rounded-md lg:rounded-md p-4 lg:p-6 space-y-2">
...
export default function DialogSentences({
  sentences,
}: {
  sentences: SentenceImprovementsType;
}) {
  const copySentences = {
    grammar: [...sentences.grammar_enhancements],
    vocabulary: [...sentences.vocabulary_enhancements],
  };
  return (
          ...
          <div className="flex flex-col gap-3 items-start justify-start">
            ...
            <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 lg:gap-6">
              {copySentences.grammar.map((item, index) => (
                <DialogImprovement improvement={item} key={index} />
              ))}
            </div>
          </div>
          <div className="flex flex-col gap-3 items-start justify-start">
