...
export function WeakSide({
  ...
  return (
    <div className="border border-amber-300 bg-white p-4 rounded-lg">
...
export default function DialogWeakSides({
  weakSides,
}: {
  weakSides: StrongPointsType;
}) {
  const weakPoints = [
    {
      title: "Fluency",
      ...
      <div className="p-6 lg:p-8 border border-gray-200 rounded-lg bg-white/60 flex flex-col gap-6 shadow-none">
        ...
        <main className="flex flex-col gap-4 lg:gap-6">
          {weakPoints.map((item, index) => (
            <WeakSide key={index} aspect={item} />
          ))}
        </main>
      </div>
    </section>
