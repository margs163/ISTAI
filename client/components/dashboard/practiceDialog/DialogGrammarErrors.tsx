...
import { GrammarAnalysisType } from "@/lib/types";
import {
  CircleCheck,
  CircleX,
  GanttChartSquare,
...
export function DialogGrammarError({
  error,
}: {
  error: {
    identified_mistakes: {
...
export default function DialogGrammarErrors({
  grammar,
}: {
  grammar: GrammarAnalysisType;
}) {
  return (
        ...
        <main className="flex flex-col gap-8">
          <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2 lg:gap-6">
            {grammar?.grammar_analysis.map((item, index) => (
              <DialogGrammarError error={item} key={index} />
            ))}
          </div>
        </main>
      </div>
